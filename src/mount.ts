...
export function startEnhancer(initialSettings: Settings = DEFAULT_SETTINGS): EnhancerStop {
  ...
  let opened: { binding: Binding; snapshot: EditorSnapshot; page: MenuPage; openAbove?: boolean } | null = null;
  ...
  function positionMenu(): void {
    ...
    const openAbove = opened.openAbove ?? (below < origin.height && above > below);
    opened.openAbove = openAbove;
    const viewportRoom = Math.max(1, bottomLimit - topLimit);
    popup.style.maxHeight = `${Math.max(1, Math.min(origin.height, openAbove ? above : below, viewportRoom))}px`;
